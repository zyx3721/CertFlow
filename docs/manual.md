...
- SSR 服务读取品牌配置使用的后端地址按以下顺序解析（默认 `http://127.0.0.1:8080`）：
  ...
  2. 入口文件所在目录向上逐级查找首个 `.env` 文件中配置的 `SSR_API_ORIGIN` 或 `VITE_API_BASE_URL`（外加进程工作目录兜底）
...
- 前端 SSR 服务渲染首屏时会按 `SSR_API_ORIGIN` 或 `VITE_API_BASE_URL`（默认 `http://127.0.0.1:8080`）直连后端读取品牌配置并直出标题、图标与启动页，后端不在该地址时需通过环境变量或入口目录向上各层的 `.env` 文件指定。
