...
# CertFlow 二进制部署在线更新脚本：分页抓取 GitHub 全部 Release，选择版本后自动完成下载校验、停止服务、替换产物并重启

REPO="zyx3721/CertFlow"
INSTALL_DIR="${INSTALL_DIR:-/data/certflow}"
...
cleanup() {
    case "$TMP_DIR" in
        /tmp/certflow-update.*) [ -d "$TMP_DIR" ] && rm -rf "$TMP_DIR" ;;
    esac
}
trap cleanup EXIT
...
# get_current_version 通过 certflow -v 读取当前已部署的版本号
get_current_version() {
    if [ -x "$INSTALL_DIR/certflow" ]; then
        CURRENT_VERSION=$("$INSTALL_DIR/certflow" -v 2>/dev/null | head -n1 | awk '{print $2}')
    fi
}
...
show_version_list() {
    ...
    if [ -n "$CURRENT_VERSION" ]; then
        echo_log_info "当前系统 CertFlow 版本为 \033[32m$CURRENT_VERSION\033[0m ，仓库 $REPO 可安装的版本有（共 $total 个）：\n"
    else
        echo_log_warn "未检测到已安装的 CertFlow 版本，仓库 $REPO 可安装的版本有（共 $total 个）：\n"
    fi
...
confirm_install() {
    ...
    while true; do
        read -rp "即将${action} CertFlow ${TARGET_TAG}${tip}，是否继续? [y/N]: " yn
        case "$yn" in
            [Yy]*) break ;;
            [Nn]*|"")
...
prepare_assets() {
    ...
    BACKEND_ASSET="certflow_${ver}_linux_${ARCH}.tar.gz"
    FRONTEND_ASSET="certflow-frontend_${ver}.tar.gz"
...
# download_packages 预检并下载前后端安装包与可选的校验和文件
download_packages() {
    echo ""
    echo_log_info "\033[32m开始下载 CertFlow ${TARGET_TAG} 安装包...\033[0m"
...
verify_packages() {
    ...
    local asset
    for asset in "$BACKEND_ASSET" "$FRONTEND_ASSET"; do
        grep "${asset}\$" "$TMP_DIR/SHA256SUMS" | (cd "$TMP_DIR" && sha256sum -c -) \
            || echo_log_error "安装包完整性校验失败: $asset"
    done
...
extract_packages() {
    ...
    [ -f "$TMP_DIR/backend/linux_${ARCH}/certflow" ] || echo_log_error "后端安装包内未找到 certflow 二进制文件"
...
install_packages() {
    ...
    if [ -f "$INSTALL_DIR/certflow" ]; then
        cp -f "$INSTALL_DIR/certflow" "$INSTALL_DIR/certflow.bak" || echo_log_error "备份后端二进制失败"
        chmod --reference="$INSTALL_DIR/certflow.bak" "$TMP_DIR/backend/linux_${ARCH}/certflow" 2>/dev/null
        chown --reference="$INSTALL_DIR/certflow.bak" "$TMP_DIR/backend/linux_${ARCH}/certflow" 2>/dev/null
    fi
    chmod +x "$TMP_DIR/backend/linux_${ARCH}/certflow"
    cp -f "$TMP_DIR/backend/linux_${ARCH}/certflow" "$INSTALL_DIR/certflow.new" || echo_log_error "写入新后端二进制失败"
    mv -f "$INSTALL_DIR/certflow.new" "$INSTALL_DIR/certflow" || echo_log_error "替换后端二进制失败"
...
rollback() {
    ...
    if [ -f "$INSTALL_DIR/certflow.bak" ]; then
        mv -f "$INSTALL_DIR/certflow.bak" "$INSTALL_DIR/certflow" && chmod +x "$INSTALL_DIR/certflow"
    fi
...
start_services() {
    ...
    [ $ok -eq 0 ] && rollback
    rm -f "$INSTALL_DIR/certflow.bak"
    rm -rf "$INSTALL_DIR/dist.bak"
    local new_version
    new_version=$("$INSTALL_DIR/certflow" -v 2>/dev/null | head -n1 | awk '{print $2}')
...
main() {
    ...
    detect_arch
    get_current_version
    TMP_DIR=$(mktemp -d /tmp/certflow-update.XXXXXX)
    fetch_all_releases
    show_version_list
    select_version
    confirm_install
