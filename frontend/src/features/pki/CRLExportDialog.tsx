...
import type { CRLEntry } from '@/lib/pki';
import { formatDate, revokeReasonLabels } from './support';
...
const columns: ExportColumn<CRLEntry>[] = [
  { id: 'commonName', header: '通用名称', value: item => item.commonName },
  { id: 'serialNumber', header: '序列号', value: item => item.serialNumber },
  { id: 'revokedAt', header: '撤销时间', value: item => formatDate(item.revokedAt, true) },
  {
    id: 'reason',
    header: '撤销原因',
    value: item => revokeReasonLabels[item.reason] ?? item.reason,
  },
  { id: 'caName', header: '所属 CA', value: item => item.caName },
  { id: 'caId', header: 'CA ID', value: item => item.caId },
];

export function CRLExportDialog({
  open,
  filteredItems,
  onOpenChange,
}: {
  open: boolean;
  filteredItems: CRLEntry[];
  onOpenChange: (open: boolean) => void;
}) {
  const [format, setFormat] = useState<ExportFormat>('xlsx');
  const [filename, setFilename] = useState(`CRL-${localTimestamp()}`);
  ...
  useEffect(() => {
    if (!open) return;
    setFormat('xlsx');
    setFilename(`CRL-${localTimestamp()}`);
  ...
  const previewName = `${sanitizeExportFileName(filename || `CRL-${localTimestamp()}`)}.${format}`;
  ...
  function handleExport() {
    ...
    toast.success(`已导出 ${filteredItems.length} 条 CRL 记录`);
    onOpenChange(false);
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader className="border-b border-[var(--zl-border)] px-5 py-4">
          <DialogTitle>导出 CRL 数据</DialogTitle>
          <DialogDescription>
            将当前筛选后的 {filteredItems.length} 条 CRL 记录进行导出
          </DialogDescription>
        </DialogHeader>
