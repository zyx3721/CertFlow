...
- **CA 管理**：支持 Root、Intermediate、Issuing CA，以及 RSA、ECDSA、ED25519 算法；可查看 PEM 证书并递归删除未关联的 CA 层级。
...
- [CertFlow 新手证书签发与 Nginx 配置指南](<docs/CertFlow新手证书签发与Nginx配置指南.md>)：从创建内部 CA、申请与审批业务证书，到部署 Nginx HTTPS 反向代理的完整操作流程。
...
- 登录页始终提供本地账号方式；只有已启用 LDAP 配置时才显示 AD/LDAP 登录方式。
...
- 在「CA 管理」页面创建 Root、Intermediate 或 Issuing CA。
- 非 Root CA 必须选择非签发类上级 CA，且子 CA 的有效期不能超过上级 CA。
- 删除某个 CA 时，系统会同时删除其全部下级 CA；只有该 CA 树未关联任何证书时才允许执行。

### 5.2.2 证书申请与审批

- 在「证书申请」页面填写 X.509 Subject、通用名称、SAN、签发 CA、算法、用途和有效期，并选择系统生成或上传已有 CSR。
...
- 手动 CSR 下载单个 `<通用名称>.crt`；系统生成模式下载 `<通用名称>.zip`，其中包含 `<通用名称>_bundle.crt`（叶子证书与直接签发 CA 证书）和匹配的私钥文件。多级 CA 场景下，可参考[新手证书签发与 Nginx 配置指南](<docs/CertFlow新手证书签发与Nginx配置指南.md>)补齐服务端中间证书链。
...
- 品牌图标可保留默认的 `/favicon.svg`，或上传不超过 256KB 的图片；LDAP Bind Password 与 SMTP Password 不会经 API 回显。

### 5.4.2 操作审计

- 操作审计记录登录、CA、证书、审批、用户及系统配置变更。
...
- 已记录到 `schema_migrations` 的迁移不会重放，因此修改已应用的 `001_init.sql` 不会改变已有数据库。

# 六、安全说明

- CA 与平台代管证书私钥采用 AES-256-GCM 加密存储。
- 会话令牌仅以 SHA-256 哈希形式保存到 PostgreSQL。
- LDAP Bind Password 与 SMTP Password 使用同一 AES-256-GCM 主密钥加密保存，API 仅返回是否已配置，不回显明文。
...
- 私钥不出现在证书列表 API；私钥下载必须使用后续的专用、授权下载接口。
